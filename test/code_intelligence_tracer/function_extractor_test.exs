defmodule CodeIntelligenceTracer.FunctionExtractorTest do
  ...
  describe "compute_complexity/1" do
    ...
    test "all functions include complexity field" do
      ...
      for {_key, func_info} <- functions do
        assert Map.has_key?(func_info, :complexity),
          "Function info should have :complexity key"
        assert is_integer(func_info.complexity) and func_info.complexity >= 1,
          "Complexity should be integer >= 1"
      end
    end
  end
