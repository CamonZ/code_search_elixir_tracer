defmodule ExAst.BeamReaderTest do
  ...
  describe "read_chunks/1" do
    test "reads chunks from valid BEAM file" do
      ...
      beam_path = get_beam_path(ExAst.BeamReader)

      assert {:ok, {module, chunks}} = BeamReader.read_chunks(beam_path)

      assert module == ExAst.BeamReader
  ...
  describe "extract_debug_info/2" do
    test "extracts debug info from Elixir module" do
      ...
      assert debug_info.module == ExAst.BeamReader
    end
  ...
  describe "collect_module_names/1" do
    test "collects module names from BEAM paths" do
      ...
      assert MapSet.member?(modules, "ExAst.BeamReader")
    end

    test "collects multiple module names" do
      beam_paths = [
        get_beam_path(ExAst.BeamReader),
        get_beam_path(ExAst.BuildDiscovery)
      ]
      ...
      assert MapSet.member?(modules, "ExAst.BeamReader")
      assert MapSet.member?(modules, "ExAst.BuildDiscovery")
    end
    ...
    test "skips invalid BEAM files" do
      ...
      valid_path = get_beam_path(ExAst.BeamReader)
